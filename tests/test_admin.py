...
def test_product_creation(auth_client, init_db):
    ...
    response = auth_client.post(
        "/admin/rekomendasi", data=data, content_type="multipart/form-data", follow_redirects=True
    )
...
def test_product_update(auth_client, init_db):
    ...
    response = auth_client.put(
        f"/admin/rekomendasi/{product.id}",
        data=dict(brand="New Brand", description="New Description", recommendations="Kering", skincareTypes="Toner"),
...
def test_feedback_deletion(auth_client, init_db):
    feedback = Feedback(content="Great app!", user_id=1)
    db.session.add(feedback)
    db.session.commit()
    response = auth_client.delete(f"/admin/feedback/{feedback.id}", follow_redirects=True)
    assert response.status_code == 200
    deleted_feedback = Feedback.query.get(feedback.id)
